...
- For repo-related work, check `.furnace/repo-index.md` before broad exploration unless the task is clearly file-specific. Treat it as a compact orientation dictionary, not source of truth. Do not update it merely because it is old. If your work changes or reveals meaningful repo-level structure, update only the relevant part of the index and `.furnace/repo-index.meta.json` when practical; do not churn it for tiny changes.
