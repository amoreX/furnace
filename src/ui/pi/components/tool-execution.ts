...
import type { ToolDefinition, ToolRenderContext } from "../tools/types.js";
import { createAllToolDefinitions, type ToolName } from "../tools/index.js";
import { getTextOutput as getRenderedTextOutput } from "../render-utils.js";
import { theme } from "../theme.js";

/**
 * pi converts non-PNG images to PNG via photon (wasm) for the kitty graphics
 * protocol. Furnace does not ship photon, so this is a no-op "conversion" that
 * returns the input unmodified. PNG/JPEG rendering in terminals whose image
 * protocol accepts them (e.g. iTerm2) is unaffected; kitty simply skips
 * non-PNG images.
 */
async function convertToPng(base64Data: string, mimeType: string): Promise<{ data: string; mimeType: string } | null> {
...
export class ToolExecutionComponent extends Container {
	...
	private formatArgsSummary(): string {
		...
		if (args && typeof args === "object") {
			...
			const primary = entries.find(([, value]) => typeof value === "string") ?? entries[0];
			if (primary) {
