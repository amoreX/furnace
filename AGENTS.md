...
- `src/commands.ts` defines built-in slash commands including `/new`, `/split` (beta split-pane TUI), `/resume`/`/history`, `/fork`, `/clone`, `/image`, `/login`, `/model`, `/plan`, `/agent`, `/mode`, `/theme`, `/tasks`, `/compact`, `/skills`, `/lofi`, `/settings`, `/permissions`, `/status`, `/export`, `/diff`, `/undo`, `/copy`, `/cost`, `/editor`, `/bug`, `/exit`, and `/quit`.
