...
- If `.furnace/repo-index.md` exists and the task touches or depends on this repository, read it first when useful as a local orientation dictionary before broader exploration. If later exploration shows it is stale, update it with the new high-to-mid level repo knowledge.
