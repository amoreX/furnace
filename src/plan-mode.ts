...
export function appendPlanModeGuidance(systemPrompt: string, state: PlanModeState): string {
  ...
  return [
    ...
    "",
    "<plan_mode>",
    "CRITICAL — PLAN MODE IS ACTIVE. The user requires research and a saved plan before any implementation.",
    "",
    "Hard rules:",
    ...
    "",
    "Plan artifact:",
    `- You MUST call the write tool to create ${planPath}, or the edit tool if that exact plan file already exists.`,
    "- A prose-only plan in your response is incomplete. Do not claim the plan is ready until the plan-file tool call succeeds.",
    `- The successful plan-file tool call must save the final recommended approach at exactly ${planPath}.`,
